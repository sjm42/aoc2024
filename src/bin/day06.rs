...
fn main() -> anyhow::Result<()> {
    ...
    let mut map_walk1 = clone_map(&map);
    let start = (pos_x.unwrap(), pos_y.unwrap());
    let direction = (0, -1);
    let (infinite, i, pos, dir) = guard_walk(&mut map_walk1, start, direction);
    println!("Walk1: Guard walked {i} steps at {pos:?} heading to {dir:?}. Infinite loop: {infinite}");
    println!("Walk1: visited positions# {}", count_visited(&map_walk1));
...
fn guard_walk(
    ...
    loop {
        ...
        if let Entry::Vacant(e) = walked.entry(key) {
            ...
            // I have been here, walking to same direction!
            return (true, i, pos, dir);
        }
        let new_pos = (pos.0 + dir.0, pos.1 + dir.1);

        if new_pos.0 < 0 || new_pos.0 >= size_x || new_pos.1 < 0 || new_pos.1 >= size_y {
            return (false, i, pos, dir);
        }
...
fn block_guard(map: &[Vec<char>], start: (i32, i32), direction: (i32, i32)) -> usize {
    ...
    for y in 0..size_y {
        for x in 0..size_x {
            ...
            let (infinite, i, _pos, _dir) = guard_walk(&mut try_map, start, direction);
            if infinite {
                info!("Found Infinite loop, blocking ({y},{x}), steps {i}");
                n_inf += 1;
            }
        }
    }
